...
public class MypageRentalFinServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<rent> list = new rentService().rentfinviewList(itemrentuser);
		int cntfin = new rentService().rentfincnt(itemrentuser);
	
		
		
		//4.뷰단 포워딩		
		RequestDispatcher reqDispatcher = request.getRequestDispatcher("/WEB-INF/views/mypage/mypageRentalFin.jsp");
